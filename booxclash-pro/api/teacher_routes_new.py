...
from services.llm_teacher_engine_new import (
    ...
    generate_chalkboard_diagram,  
    evaluate_lesson_feedback      
)
...
class WeeklyPlanRequest(BaseModel):
    ...
    weekNumber: int
    days: Optional[int] = 5
    startDate: Optional[str] = "2026-01-12"
    schoolId: Optional[str] = None
...
def resolve_user_id(header_uid: Optional[str], body_uid: str) -> str:
    user_id = header_uid or body_uid
    if not user_id: raise HTTPException(status_code=400, detail="Missing user ID")
    return user_id
...
def find_cached_document(collection_name: str, grade: str, subject: str, term: str = None, topic: str = None, subtopic: str = None, doc_type: str = "lesson", requested_topics: List[str] = None, requested_subtopics: List[str] = None):
    ...
    print(f"\n🔍 [CACHE ENGINE NEW] Searching for {doc_type.upper()}...")
    ...
    for col in collections_to_check:
        ...
        try:
            ...
            for doc in docs:
                ...
                # 🎯 MATCHING FOR LESSONS, WEEKLY, & RECORDS
                if doc_type != "scheme":
                ...
                if doc_type == "record": return data.get("recordData", data.get("data", data))
...
async def generate_scheme(
    ...
    if db_cached_plan:
        print("✅ [SMART CACHE] Match found! Swapping context locally...")
...
async def generate_weekly(
    ...
    clean_topic = request.topic or getattr(request, "theme", "")
    ...
    if db_cached_plan:
        ...
        ai_weekly = swap_context_locally(db_cached_plan, {
            "Teacher Name": getattr(request, "teacherName", "Teacher"),
            "School Name": getattr(request, "school", "Unknown School"),
        ...
        print("🤖 [AI GEN] Generating fresh Weekly Plan from AI...")
        try:
            ai_weekly = await generate_weekly_plan_from_scheme(
                scheme_row={
                    "topic": clean_topic,
                    "subtopic": getattr(request, "subtopic", getattr(request, "lessonTitle", "")),
                ...
                days_in_week=getattr(request, "days", 5)
            )
        except Exception as e:
            traceback.print_exc()
    ...
    if ai_weekly:
        try:
            save_weekly_plan(
                ...
                data=ai_weekly, school_id=school_id 
            )
        except Exception as save_error:
            print(f"⚠️ Could not save weekly plan to DB: {save_error}")
...
async def generate_lesson(
    ...
    syllabus_data = load_module("Zambia", request.grade, request.subject)

    # ♻️ CHECK CACHE
    db_cached_plan = find_cached_document(
        "generated_lesson_plans", request.grade, request.subject, 
        topic=request.topic, subtopic=getattr(request, "subtopic", ""), doc_type="lesson"
    )

    if db_cached_plan:
        print("🔄 [CONTEXT SWAP] Applying new names instantly via Python...")
        ai_lesson = swap_context_locally(db_cached_plan, {
            "Teacher Name": getattr(request, "teacherName", "Teacher"),
            "School Name": getattr(request, "school", "Unknown School"),
        ...
        print("🤖 [AI GEN] Generating fresh Lesson Plan from AI...")
        try:
            ai_lesson = await generate_specific_lesson_plan(
                subject=request.subject,
                grade=request.grade,
                topic=request.topic,
                subtopic=getattr(request, "subtopic", ""),
                duration=40,
                boys=getattr(request, "boys", 0),
                girls=getattr(request, "girls", 0),
                syllabus_context=syllabus_data
            )
        except Exception as e:
            traceback.print_exc()
            raise HTTPException(status_code=500, detail=str(e))
    ...
    if ai_lesson:
        try:
            save_lesson_plan(
                uid=uid, subject=request.subject, grade=request.grade,
                data=ai_lesson, term=request.term, school_name=request.school,
...
async def generate_record(
    ...
    # ♻️ CHECK CACHE
    db_cached_plan = find_cached_document("generated_records_of_work", request.grade, request.subject, topic=request.topic, doc_type="record")

    if db_cached_plan:
        print("🔄 [CONTEXT SWAP] Applying new names instantly via Python...")
        ai_record = swap_context_locally(db_cached_plan, {
            "Teacher Name": getattr(request, "teacherName", "Teacher"),
            "School Name": getattr(request, "school", "Unknown School"),
            "Week": request.weekNumber,
            "Start Date": getattr(request, "startDate", "")
        ...
        print("🤖 [AI GEN] Generating fresh Record of Work from AI...")
        try:
            ai_record = await generate_record_of_work(
                scheme_row={
                    "topic": request.topic,
                    "subtopic": getattr(request, "subtopic", ""),
                    "objectives": getattr(request, "objectives", []),
    ...
    if ai_record:
        try:
            save_record_of_work(
                uid=uid, subject=request.subject, grade=request.grade,
                term=request.term, week_number=request.weekNumber, 
                school_name=request.school, data=ai_record
            )
        except Exception as save_error:
            print(f"⚠️ Could not save record of work to DB: {save_error}")
    ...
    try:
        ...
        if "scheme" in request.planType.lower():
            ...
            if isinstance(rows_to_extract, list):
                ...
                print(f"✅ Extracted Topics for Flywheel: {training_record.get('topics')}")
...
async def evaluate_lesson_endpoint(
...
async def generate_diagram(
    ...
    uid = resolve_user_id(x_user_id, request.uid)
    print(f"🎨 Generating Diagram | Prompt: {request.prompt[:30]}...")

    try:
        result = await generate_chalkboard_diagram(
