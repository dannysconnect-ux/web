...
import { Printer, ChevronLeft, CheckCircle, FileSignature, ImagePlus, Loader2, X } from 'lucide-react';
...
export default function ExamView() {
  ...
  const examTitle = examData?.exam_title || `${meta?.grade} ${meta?.subject} Test`;

  useEffect(() => {
    if (!examData) navigate('/');
  }, [examData, navigate]);

  if (!examData || !meta) return null;
  ...
  if (examData.multiple_choice?.length) activeSections.push({ type: 'mcq', title: 'Multiple Choice', data: examData.multiple_choice });
  if (examData.true_false?.length) activeSections.push({ type: 'tf', title: 'True or False', data: examData.true_false });
  if (examData.matching?.length) activeSections.push({ type: 'matching', title: 'Matching Pairs', data: examData.matching });
  if (examData.short_answer?.length || examData.one_word?.length) activeSections.push({ type: 'short', title: 'Short Answer', data: examData.short_answer || examData.one_word });
  if (examData.computational?.length) activeSections.push({ type: 'comp', title: 'Problem Solving', data: examData.computational });
  if (examData.case_study?.length) activeSections.push({ type: 'case', title: 'Case Study', data: examData.case_study });
  if (examData.essay?.length) activeSections.push({ type: 'essay', title: 'Essay / Long Answer', data: examData.essay });
  ...
  const handleDownloadPDF = async () => {
    ...
    const drawPdfImageIfAny = async (qId: string) => {
        ...
        if (imgData) {
            try {
                // If it's a generated base64 from the backend, use it directly. Otherwise, fetch it.
                ...
                if (base64) {
                    checkPageBreak(50);
                    const imgW = 60, imgH = 60; // Max bounds
    ...
    for (const section of sectionsWithMeta) {
        ...
        for (let i = 0; i < section.data.length; i++) {
            ...
            if (section.type === 'mcq') {
                ...
                await drawPdfImageIfAny(`${section.type}_${i}`); // INJECT IMAGE
                ...
                doc.text(`[${q.points_allocated} marks]`, pageWidth - margin - 20, yPos); doc.setFont("times", "normal");
  ...
  const renderImageTools = (q: any, qId: string) => {
    ...
    const defaultPrompt = q.image_prompt || q.question || "Diagram";

    return (
        <div className="relative group/tool">
            {!images[qId] && (
                <div className={`ml-6 mt-2 print:hidden transition-all duration-300 ${
                    aiSuggested ? 'opacity-100 block' : 'opacity-0 h-0 overflow-hidden group-hover/tool:opacity-100 group-hover/tool:h-auto group-hover/tool:block'
                }`}>
                    <button 
                    ...
                    >
                        ...
                        {loadingImages[qId] ? "Searching..." : aiSuggested ? `Generate Diagram (${q.image_prompt})` : "Add Diagram Here"}
                    </button>
                </div>
            )}
            {images[qId] && (
                <div className="ml-6 mt-3 mb-4 print:ml-0 print:mt-4 print:mb-4 flex flex-col items-start relative group/img">
                    <button 
                        ...
                        className="absolute -top-3 -right-3 bg-red-100 text-red-600 p-1.5 rounded-full hover:bg-red-200 print:hidden opacity-0 group-hover/img:opacity-100 transition-opacity z-10"
                        title="Remove Image"
                    ><X size={14} /></button>
                    
                    {/* Simplified Image Rendering for both URLs and Base64 Data URLs */}
                    <img 
                        src={images[qId].data} 
                        alt="Exam Diagram" 
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans pb-20 print:bg-white print:pb-0 flex flex-col items-center">
      ...
      <div className="w-full max-w-[210mm] bg-white sm:min-h-[297mm] sm:my-8 p-6 sm:p-12 sm:shadow-2xl rounded-sm print:m-0 print:p-0 print:shadow-none print:w-full font-serif">
                    ...
                    <section key={section.type} className="pt-2">
                        ...
                        <div className="space-y-6">
                            ...
                            {section.type === 'mcq' && section.data.map((q: any, i: number) => {
                                ...
                                return (
                                <div key={i} className="break-inside-avoid group/tool">
                            ...
                            {section.type === 'matching' && section.data.map((block: any, i: number) => {
                                ...
                                return (
                                <div key={i} className="break-inside-avoid bg-slate-50 p-4 rounded-xl border border-slate-200 print:bg-transparent print:border-none print:p-0 group/tool">
                                    <p className="italic text-slate-700 mb-4">{block.instruction}</p>
                            ...
                            {section.type === 'case' && section.data.map((cs: any, i: number) => (
                                <div key={i} className="break-inside-avoid group/tool mb-8">
                                    <div className="bg-slate-50 border border-slate-200 p-4 sm:p-6 rounded-lg mb-6 text-sm leading-relaxed print:border-slate-400"><span className="font-bold uppercase text-xs text-slate-500 mb-2 block tracking-widest">Scenario</span>{cs.scenario}</div>
                                    ...
                                    <div className="space-y-6 pl-4 border-l-2 border-slate-200 print:border-transparent">
                                        {cs.questions.map((q: any, qIdx: number) => {
                                            const currentQ = qNumReact++;
                                            return <div key={qIdx} className="group/subtool"><p className="font-semibold text-slate-900 flex gap-2"><span>{currentQ}.</span><span>{q.question}</span></p>{renderImageTools(q, `${section.type}_${i}_q_${qIdx}`)}{showAnswers ? <div className="pl-6 text-emerald-700 font-bold italic print:text-black mt-2">Answer: {q.answer}</div> : <div className="pl-6 space-y-4 pt-2"><div className="border-b border-slate-400 w-full border-dotted"></div><div className="border-b border-slate-400 w-full border-dotted"></div></div>}</div>
                                        })}
                                    </div>
                                </div>
                            ))}
        ...
        <div className="mt-12 pt-4 border-t border-slate-300 text-center text-[10px] text-slate-500 font-medium font-sans">
           End of Test • Created by {teacherName}
        </div>
      </div>
    </div>
  );
